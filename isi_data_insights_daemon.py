...
class DerivedStatsProcessor(object):
    ...
    def begin_process(self):
        for derived_stat_computer in self._derived_stat_computers:
            derived_stat_computer.begin_process()
    ...
    def end_process(self):
        for derived_stat_computer in self._derived_stat_computers:
            derived_stat_computer.end_process()
...
class PercentChangeStatComputer(DerivedStatComputer):
    ...
    def begin_process(self):
        super(PercentChangeStatComputer, self).begin_process()
        self._prev_values = self._cur_values
        self._cur_values = {}
    ...
    def compute_derived_stats(self):
        ...
        for node in self._cur_values:
            ...
            if cur_value is None:
                ...
                try:
                    prev_value = self._prev_values[node]
                    LOG.debug("PCS [%s]=(%s /  %s) - 1",
                            str(node), str(cur_value), str(prev_value))
                    try:
                        derived_stat_value = (cur_value / prev_value) - 1
                    except ZeroDivisionError:
                        derived_stat_value = cur_value
...
class IsiDataInsightsDaemon(run.RunDaemon):
    ...
    def _process_stats_with_derived_stats(
        ...
        (cluster_composite_stats, equation_stats, pct_change_stats) = \
                derived_stats
        cluster_composite_stats.begin_process()
        equation_stats.begin_process()
        pct_change_stats.begin_process()
        # process the results
        ...
        for eq_stat in equation_stats.stats():
            # equation stats might produce more than one one derived,
        ...
        cluster_composite_stats.end_process()
        equation_stats.end_process()
        pct_change_stats.end_process()
